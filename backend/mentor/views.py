...
class MentorFileUploadAPIView(APIView):
    ...
    def put(self, request):
        ...
        if cv_file:
            ...
            if not any(cv_file.name.lower().endswith(ext) for ext in valid_extensions):
                return Response({"error": "Invalid file type. Only PDF, DOC, and DOCX are allowed."}, status=status.HTTP_400_BAD_REQUEST)
            if mentor.details.cv:
                mentor.details.cv.delete()
            mentor.details.cv = cv_file
            mentor.details.save()
            return Response({"cv_url": mentor.details.cv.url}, status=status.HTTP_200_OK)

        elif profile_photo:
            valid_extensions = ['.jpg', '.jpeg', '.png', '.gif']
            if not any(profile_photo.name.lower().endswith(ext) for ext in valid_extensions):
                return Response({"error": "Invalid file type. Only JPG, JPEG, PNG, and GIF are allowed."}, status=status.HTTP_400_BAD_REQUEST)
            if mentor.details.profile_photo:
                mentor.details.profile_photo.delete()
            mentor.details.profile_photo = profile_photo
            mentor.details.save()
            return Response({"profile_photo_url": mentor.details.profile_photo.url}, status=status.HTTP_200_OK)
    ...
    def delete(self, request):
        ...
        if file_type == 'cv' and mentor.details.cv:
            ...
            mentor.details.save()
            return Response(status=status.HTTP_204_NO_CONTENT)
        return Response({"error": f"No {file_type} to delete"}, status=status.HTTP_400_BAD_REQUEST)




# done for fetching mentors in cards and search by category   done by rittul
@api_view(['GET'])
@permission_classes([AllowAny])
