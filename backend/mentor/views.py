...
#     def post(self, request):
#         student = request.user  # Custom user model is Student

#         mentor_id = request.data.get("mentor_id")
#         subject = request.data.get("subject")
#         time_slot = request.data.get("time_slot")  # string
#         transaction_id = request.data.get("transaction_id")

#         # Parse string to datetime
#         scheduled_time = parse_datetime(time_slot)
#         if scheduled_time and is_naive(scheduled_time):
#             scheduled_time = make_aware(scheduled_time)

#         if not all([mentor_id, subject, scheduled_time, transaction_id]):
...
#         if Meeting.objects.filter(student=student, mentor=mentor, scheduled_time=scheduled_time).exists():
#             return Response({"error": "You have already booked this slot with the mentor."}, status=status.HTTP_400_BAD_REQUEST)
