...
const ProfilePage = ({ mentorProfile }) => {
  ...
  const buildProfilePayload = (profileData) => {
    ...
    if (profileData.dob) details.dob = profileData.dob;
    if (profileData.age) details.age = parseInt(profileData.age);
    if (profileData.gender) details.gender = profileData.gender.toLowerCase();
    if (profileData.phone) details.phone_number = profileData.phone;
    if (profileData.college) details.college = profileData.college;
    if (profileData.cgpa) details.cgpa = parseFloat(profileData.cgpa);
    if (profileData.batch) details.batch = parseInt(profileData.batch);

    if (profileData.expertise) {
      details.professions = [{ title: profileData.expertise }];
    }
    ...
    if (profileData.skills) {
      details.skills = profileData.skills
        ...
        .filter((skill) => skill.name);  // remove empty names
    }

    if (profileData.hourlyRate) details.fees = profileData.hourlyRate;
    if (profileData.bio) details.about = profileData.bio;
    if (profileData.availability) details.availability_timings = profileData.availability;
    if (profileData.yearsOfExperience) details.years_of_experience = parseInt(profileData.yearsOfExperience);
    if (profileData.linkedin) details.linkedin_url = profileData.linkedin;
    if (profileData.github) details.github_url = profileData.github;
    if (profileData.portfolio) details.portfolio_url = profileData.portfolio;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        'http://127.0.0.1:8000/api/student/profile/update/', // Update this URL to match your Django endpoint
        payload,
        {
          headers: {
  ...
  const handleRemoveCV = async () => {
    try {
      await axios.delete(
        'http://127.0.0.1:8000/api/student/profile/file/', // Update this URL
        {
          headers: {
            'Authorization': `Token ${getAuthToken()}`
          }
