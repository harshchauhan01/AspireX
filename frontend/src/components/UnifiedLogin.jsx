...
function UnifiedLogin() {
  const [form, setForm] = useState({ user_id: '', password: '', role: 'student' });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const res = await API.post('auth/unified-login/', requestData);
  ...
  const handleGoogleSignIn = useCallback(async (response) => {
    ...
    setError('');
    setError('🔄 Authenticating with Google... Please wait.');
      ...
      <StyledWrapper>
        <div className="container">
          ...
          <form className="form" onSubmit={handleSubmit}>
            <input 
              placeholder={form.role === 'mentor' ? 'Mentor ID' : 'Student ID'} 
              name="user_id" 
              type="text" 
              className="input" 
            ...
            <input 
              defaultValue={isLoading ? "Signing In..." : "Sign In"} 
              type="submit" 
